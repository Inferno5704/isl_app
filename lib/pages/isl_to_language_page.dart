...
class _SignLanguageDetectorState extends State<SignLanguageDetector> with WidgetsBindingObserver {
  ...
  static const Duration _processingInterval = Duration(milliseconds: 500); // Adjust as needed
  ...
  Future<void> _processImage(CameraImage image) async {
    ...
    try {
      // Faster preprocessing with reduced steps
  ...
  int _findMaxIndexWithConfidence(List<double> list, {double threshold = 0.5}) {
    ...
    // Only return index if confidence is above threshold
  ...
  dynamic _fastPreprocessImage(CameraImage image) {
    try {
      // Direct conversion and normalization
      ...
      for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
          ...
          // Faster YUV to RGB with direct scaling
